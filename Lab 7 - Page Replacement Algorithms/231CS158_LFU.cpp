// C++ program to simulate LRU Paging
#include <iostream>
...
void LRUPaging(int page)
{
    ...
    // If the page does not exist in a frame, find the Least Recently Used page to remove
    for (int j=0; j < capacity; j++) {
        if (pages[frames[j]] < lru_t) {
            lru_t = pages[frames[j]];
            val = j;
        }
    }
